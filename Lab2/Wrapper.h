...
public:
	...
	void SetData(TMemo *Memo1, int n) {
		Student *tmp = new Student[n];

		for (int j = 0, k = 0; j < Memo1->Lines->Count; k++) {
			tmp[k].fio = Memo1->Lines->Strings[j];
			tmp[k].specialty = Memo1->Lines->Strings[j + 1];
			tmp[k].eduform = Memo1->Lines->Strings[j + 2];
			tmp[k].gpa = 0;
			for (int i = 0; i < 3; i++) {
				tmp[k].marks[i] = StrToInt(Memo1->Lines->Strings[j + 3 + i]);
				tmp[k].gpa += tmp[k].marks[i];
